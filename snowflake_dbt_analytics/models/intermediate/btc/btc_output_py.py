...
def model(dbt, session):
    ...
    df = dbt.ref("btc").to_pandas()

    df["OUTPUTS"] = df["OUTPUTS"].apply(simplejson.loads)
    df_exploded = df.explode("OUTPUTS").reset_index(drop=True)
    df_output = pandas.json_normalize(df_exploded["OUTPUTS"])[["address", "value"]]
    df_final = pandas.concat([df_exploded.drop(columns=["OUTPUTS"]), df_output], axis=1)
